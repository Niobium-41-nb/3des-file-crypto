...
│   └── gui.cpp          图形界面（纯 Win32 API，模仿 7-Zip 主窗口，含自绘工具栏/图标与 DPI 自适应）
├── tests/
...
命令行版适合脚本与批量处理，日常使用推荐图形界面。双击 `tdes_gui.exe` 即可。
界面整体模仿 **7-Zip 的主窗口**，自上而下分为五层：

```
 文件(F)  编辑(E)  查看(V)  工具(T)  帮助(H)                    ← 菜单栏
+------------------------------------------------------------+
| [打开] [加密] [解密] | [信息] [随机密钥] [测试] | [关于]       |  ← 工具栏（图标全部由 GDI 自绘）
+------------------------------------------------------------+
| [↑]  D:\文档\report.docx                              [v]  |  ← 地址栏（可输入路径，下拉为目录层级）
+------------------------------------------------------------+
| 名称              |   大小  | 修改日期         |属性| 类型    |  ← 文件列表（ListView，可点列头排序）
| ..                                                          |
| report.docx       |  15,037 | 2026-09-23 21:42 |  A | 文本文档   |
| report.docx.3des  |  15,060 | 2026-09-23 21:43 |  A | 3DES 加密文件|
+------------------------------------------------------------+
| 使用说明 / 加解密结果日志（查看菜单可折叠）                     |
+------------------------------------------------------------+
| 2 个对象          |      30,097 字节       |  加密完成        |  ← 状态栏（三格）
+------------------------------------------------------------+
```
...
快捷键：`Ctrl+O` 打开、`Ctrl+E` 加密、`Ctrl+D` 解密、`Ctrl+K` 随机密钥、`F5` 刷新列表、`F1` 使用说明。

细节说明：

| 项目 | 说明 |
| --- | --- |
| 输出路径 | 默认预填：加密为 `<原文件名>.3des`，解密为去掉 `.3des` 的原名（无该后缀则追加 `.dec`）；可在对话框里直接编辑，或用“浏览…”改到任意目录 |
| 输出路径校验 | 输出目录不存在、或输出文件与输入文件是同一路径时，对话框会当场提示并拒绝执行，避免原文件被覆盖损坏 |
| 覆盖保护 | 输出文件已存在时先弹确认框（对话框里可取消该确认） |
| 解密参数 | 只需密钥：算法、模式、IV 都记录在密文文件头里，会自动读取 |
| 错误提示 | 密钥不对（PKCS#7 校验失败）会弹错误框，并在日志面板中记录原因 |
| 密钥处理 | 密钥只保存在内存里（不落盘），程序退出即消失；对话框会带出本次运行已用过的密钥，方便“加密后立刻解密” |
| 高分屏 | 界面按系统 DPI 自动缩放（100% / 150% / 200% 均清晰） |
| 图标 | 窗口 / 任务栏 / 资源管理器使用 `assets/icon.ico`（多尺寸 16~256，`tools/make_icon.py` 可重新生成），exe 同时带版本信息（产品名/版本/版权）；工具栏与文件列表的小图标由 GDI **现场绘制**，不依赖任何图片资源 |
| 实现要点 | 工具栏是自绘窗口类（悬停/按下立体反馈 + 工具提示）；两个参数对话框用 `DialogBoxIndirectParamW` + **内存中的 DLGTEMPLATE** 构造，因此不需要 .rc 资源，窗口类就是标准 `#32770` |
| 依赖 | 只用 Win32 API（user32 / gdi32 / shell32 / comdlg32 / comctl32），无任何第三方 GUI 库 |
