...
| 图形界面 | `tdes_gui.exe`：纯 Win32 API 窗口界面，选文件/密钥/算法/模式后一键加解密；**输出路径可自定义**（预填默认名、可直接编辑或用“另存为…”选到任意目录）；支持拖放与高分屏 |
...
```
输入文件: [________________________________] [浏览…]
输出文件: [________________________________] [另存为…]
密钥:   [________________________________] [随机生成]
...
1. 点击“浏览…”选择**输入文件**，或**直接把文件拖进窗口**；
2. **输出文件**一栏已自动填好默认名，需要时可直接编辑，或点“另存为…”保存到任意目录/文件名；
   **清空该栏即恢复自动命名**（默认名：加密 = `<原文件名>.3des`，解密 = 去掉 `.3des` 的原名）；
3. 输入十六进制密钥，或点“随机生成”得到一个随机密钥；
4. 选好算法与模式，点“加密”或“解密”。
...
| 项目 | 说明 |
| --- | --- |
| 输出路径 | 默认预填：加密为 `<原文件名>.3des`，解密为去掉 `.3des` 的原名（无该后缀则追加 `.dec`）。可手工编辑，或用“另存为…”改到任意目录；**选择/拖入输入文件时会自动刷新默认名，一旦自定义就不再被覆盖** |
| 输出路径校验 | 输出目录不存在、或输出文件与输入文件是同一路径时会弹提示并取消操作，避免原文件被覆盖损坏 |
| 默认名等价性 | 不指定输出路径时行为与命令行版 `tdes enc <in> <out>` 完全一致，不影响原有脚本与测试 |
| 覆盖保护 | 输出文件已存在时会先弹确认框 |
...
powershell -NoProfile -ExecutionPolicy Bypass -File tools\gui_smoke.ps1       # 图形界面端到端 15 项（含输出路径）
```
