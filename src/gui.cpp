...
const int LABEL_W     = 46;   // 标签宽度
const int BTN_SMALL_W = 104;  // “浏览…/随机生成”按钮宽度
const int ROW1_Y      = 14;
const int ROW2_Y      = 48;
const int ROW3_Y      = 82;
const int ROW4_Y      = 118;
const int ACTION_H    = 38;   // 加密/解密按钮高度
const int LOG_Y       = 170;  // 日志区起始 y
const int STATUS_H    = 20;
const int MIN_W       = 520;
const int MIN_H       = 420;

HWND g_hFile = nullptr;    // 文件路径输入框
...
void layout(HWND hwnd, int w, int h) {
    ...
    if (g_hFile) MoveWindow(g_hFile, S(LABEL_W + MARGIN), S(ROW1_Y), editW, S(24), TRUE);
    if (g_hBrowse) MoveWindow(g_hBrowse, smallX, S(ROW1_Y - 1), S(BTN_SMALL_W), S(26), TRUE);
    if (g_hKey) MoveWindow(g_hKey, S(LABEL_W + MARGIN), S(ROW2_Y), editW, S(24), TRUE);
    if (g_hGenKey) MoveWindow(g_hGenKey, smallX, S(ROW2_Y - 1), S(BTN_SMALL_W), S(26), TRUE);
    if (g_hLog) {
...
void onEncrypt(HWND hwnd) {
    ...
    const std::wstring outPath = path + L".3des";
...
void onDecrypt(HWND hwnd) {
    ...
    // 推导输出文件名：去掉 .3des 后缀，否则追加 .dec
    std::wstring outPath = path;
    const std::wstring suffix = L".3des";
    if (endsWithNoCase(outPath, suffix)) {
        outPath.resize(outPath.size() - suffix.size());
    } else {
        outPath += L".dec";
    }
    if (fileExists(outPath) &&
...
LRESULT CALLBACK wndProc(HWND hwnd, UINT msg, WPARAM wp, LPARAM lp) {
    switch (msg) {
        case WM_CREATE: {
            ...
            makeControl(hwnd, L"STATIC", L"文件:", SS_LEFT, S(MARGIN), S(ROW1_Y + 4), S(LABEL_W), S(20), -1);
            ...
            makeControl(hwnd, L"STATIC", L"密钥:", SS_LEFT, S(MARGIN), S(ROW2_Y + 4), S(LABEL_W), S(20), -1);
            g_hKey = makeControl(hwnd, L"EDIT", L"", WS_BORDER | ES_AUTOHSCROLL | ES_UPPERCASE, 0,
                                 S(ROW2_Y), 100, S(24), IDC_EDIT_KEY);
            g_hGenKey = makeControl(hwnd, L"BUTTON", L"随机生成", BS_PUSHBUTTON, 0, S(ROW2_Y), 100, S(26),
                                    IDC_BTN_GENKEY);

            makeControl(hwnd, L"STATIC", L"算法:", SS_LEFT, S(MARGIN), S(ROW3_Y + 4), S(LABEL_W), S(20), -1);
            g_hAlg = makeControl(hwnd, L"COMBOBOX", L"", CBS_DROPDOWNLIST | WS_VSCROLL, S(LABEL_W + MARGIN),
                                 S(ROW3_Y), S(160), S(200), IDC_COMBO_ALG);
            ...
            makeControl(hwnd, L"STATIC", L"模式:", SS_LEFT, S(230), S(ROW3_Y + 4), S(44), S(20), -1);
            g_hCbc = makeControl(hwnd, L"BUTTON", L"CBC", BS_AUTORADIOBUTTON | WS_GROUP, S(278),
                                 S(ROW3_Y), S(62), S(22), IDC_RADIO_CBC);
            g_hEcb = makeControl(hwnd, L"BUTTON", L"ECB", BS_AUTORADIOBUTTON, S(348), S(ROW3_Y), S(62),
                                 S(22), IDC_RADIO_ECB);
            SendMessageW(g_hCbc, BM_SETCHECK, BST_CHECKED, 0);

            g_hEnc = makeControl(hwnd, L"BUTTON", L"加密 →", BS_DEFPUSHBUTTON, S(LABEL_W + MARGIN),
                                 S(ROW4_Y), S(170), S(ACTION_H), IDC_BTN_ENC);
            g_hDec = makeControl(hwnd, L"BUTTON", L"← 解密", BS_PUSHBUTTON, S(LABEL_W + MARGIN + 190),
                                 S(ROW4_Y), S(170), S(ACTION_H), IDC_BTN_DEC);
            ...
            appendLog(L"使用说明：\n"
                      L"  1) 点击“浏览…”选择文件，或把文件直接拖到本窗口；\n"
                      L"  2) 输入十六进制密钥，或点击“随机生成”得到随机密钥；\n"
                      L"  3) 选择算法与工作模式后点击“加密”或“解密”。\n"
                      L"加密结果保存为 <原文件名>.3des；解密时自动去掉该后缀还原文件名。\n"
                      L"解密只用密钥即可：算法、工作模式与 IV 都记录在密文文件头中。");
